...
export function BestSellersSection() {
  ...
  useEffect(() => {
    let mounted = true;
    // Fetch products ordered by popularity or price/created
    api
      .getProducts({ limit: 12, sortBy: 'price', sortOrder: 'desc' })
      .then((res) => {
  ...
  return (
    <section
      id="best-sellers"
      aria-label="Best Sellers Merchandising"
      className="w-full py-14 sm:py-18 bg-[var(--warm-50)]"
    >
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-8 sm:mb-10 pb-3 border-b border-neutral-200">
          <div>
            ...
            <p className="text-xs sm:text-sm text-neutral-500 mt-1 max-w-lg">
              The most loved textiles, authentic handloom sarees, and top-ordered everyday garments.
            </p>
          </div>
        ...
        {loading ? (
          <div className="ecommerce-product-grid grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 min-[1800px]:grid-cols-6 gap-3.5 sm:gap-5 lg:gap-6">
            {Array.from({ length: 10 }).map((_, i) => (
              <div
                key={i}
                className="aspect-[3/4] w-full rounded-xl bg-neutral-200/60 animate-pulse border border-neutral-200/80"
              />
            ))}
          </div>
        ) : products.length > 0 ? (
          <div className="ecommerce-product-grid grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 min-[1800px]:grid-cols-6 gap-3.5 sm:gap-5 lg:gap-6">
            {products.map((product, idx) => (
              <ProductCard key={product.id} product={product} index={idx} isBestseller={true} />
            ))}
          </div>
        ) : null}
      </div>
