...
export function WomensWearSection() {
  ...
  useEffect(() => {
    let mounted = true;
    api
      .getProducts({ categorySlug: 'women', limit: 8 })
      .then((res) => {
        if (mounted) {
          setProducts(res.products || []);
        }
      })
      .catch((err) => console.error('Failed to fetch women wear products:', err))
      .finally(() => {
  ...
  return (
    <section
      id="womens-wear"
      aria-label="Women's Wear Collection"
      className="w-full py-12 sm:py-16 bg-neutral-50/50 border-t border-b border-neutral-100"
    >
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => scroll('left')}
                aria-label="Previous women's wear products"
              ...
              <button
                ...
                aria-label="Next women's wear products"
                className="w-10 h-10 rounded-full border border-neutral-300 bg-white text-neutral-700 hover:bg-neutral-100 hover:border-neutral-400 flex items-center justify-center transition-all shadow-sm active:scale-95 cursor-pointer"
              >
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
